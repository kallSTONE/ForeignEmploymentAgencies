...
export default function AdminDashboard() {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      <h1 className="font-heading text-xl sm:text-2xl font-bold text-foreground mb-5 sm:mb-6">Dashboard Overview</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 mb-8">
        {[
          ...
          <div key={s.label} className="card-corporate flex items-center gap-3 sm:gap-4 p-4 sm:p-6">
            <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-muted flex items-center justify-center ${s.color}`}>
              <s.icon className="w-5 h-5 sm:w-6 sm:h-6" />
            </div>
            <div>
              <div className="font-heading text-xl sm:text-2xl font-bold text-foreground">{s.value}</div>
